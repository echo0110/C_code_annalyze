...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_agent_oam_init_ind->num_process] should be in range [%d <= OAM_AGENT_MAX_NUM_PROCESS] !", p_agent_oam_init_ind->num_process);
        return OAM_FAILURE;
    }
...
)
{
    ...
    OAM_CP_PACK_UINT8(*pp_buffer, &p_agent_oam_init_ind->num_process, "num_process");
    *pp_buffer += sizeof(p_agent_oam_init_ind->num_process);
    ...
    /* Compose OCTET_STRING VARIABLE of IEs */
    {
        ...
        for (loop = 0; loop < p_agent_oam_init_ind->num_process; loop++)
        {
            if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_info_list(pp_buffer, &p_agent_oam_init_ind->process_list[loop]))
            {
                return OAM_FAILURE;
            }
        }
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_dir_path(pp_buffer, &p_agent_oam_init_ind->bin_path))
    {
        return OAM_FAILURE;
    }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):AGENT_OAM_INIT_IND Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    /* Allocate buffer */
    if (OAM_FAILURE == oam_mem_alloc(msg_api_length, (void*)&p_msg, &err))
    {
        /* Not enough memory */
        return OAM_FAILURE;
    }
    ...
    /* Fill CSPL header */
    oam_construct_api_header(p_agnt_msg, OAM_VERSION_ID, src_module_id,
        dest_module_id, AGENT_OAM_INIT_IND, msg_api_length);

    /* Fill interface header */
    p_agnt_msg = p_agnt_msg + OAM_CSPL_HDR_SIZE;
    oam_construct_interface_api_header(p_agnt_msg, transaction_id, src_module_id,
        dest_module_id, AGENT_OAM_INIT_IND, msg_length,instance_id);

    /* Fill AGENT_OAM_INIT_IND message */
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_agent_oam_init_ind(&p_agnt_msg, p_agent_oam_init_ind))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_oam_agent_process_name->len] should be in range [%d <= OAM_AGENT_MAX_PROCESS_NAME] !", p_oam_agent_process_name->len);
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT8(*pp_buffer, &p_oam_agent_process_name->len, "len");
    *pp_buffer += sizeof(p_oam_agent_process_name->len);
    ...
    /* Compose OCTET_STRING VARIABLE of basic type elements */
    {
        ...
        {
            OAM_CP_PACK_CHAR8(*pp_buffer, &p_oam_agent_process_name->name[loop], "name[]");
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_name(pp_buffer, &p_oam_agent_process_info_list->process_name))
    {
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT64(*pp_buffer, &p_oam_agent_process_info_list->cksum, "cksum");
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_oam_agent_dir_path->len] should be in range [%d <= OAM_AGENT_MAX_DIR_PATH] !", p_oam_agent_dir_path->len);
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT16(*pp_buffer, &p_oam_agent_dir_path->len, "len");
    *pp_buffer += sizeof(p_oam_agent_dir_path->len);
    ...
    /* Compose OCTET_STRING VARIABLE of basic type elements */
    {
        ...
        {
            OAM_CP_PACK_CHAR8(*pp_buffer, &p_oam_agent_dir_path->name[loop], "name[]");
            *pp_buffer += sizeof(Char8);
        }
    }
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_oam_agent_spawn_process_req->num_process] should be in range [%d <= OAM_AGENT_MAX_NUM_PROCESS] !", p_oam_agent_spawn_process_req->num_process);
        return OAM_FAILURE;
    }
...
)
{
    ...
    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT8(*pp_buffer, &p_oam_agent_spawn_process_req->num_process, "num_process");
    *pp_buffer += sizeof(p_oam_agent_spawn_process_req->num_process);
    ...
    /* Compose OCTET_STRING VARIABLE of IEs */
    {
        ...
        for (loop = 0; loop < p_oam_agent_spawn_process_req->num_process; loop++)
        {
            if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_spawn_list(pp_buffer, &p_oam_agent_spawn_process_req->process_list[loop]))
            {
                return OAM_FAILURE;
            }
        }
    ...
    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT16(*pp_buffer, &p_oam_agent_spawn_process_req->interval, "interval");
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):OAM_AGENT_SPAWN_PROCESS_REQ Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_spawn_process_req(&p_agnt_msg, p_oam_agent_spawn_process_req))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_oam_agent_process_args->len] should be in range [%d <= OAM_AGENT_PROCESS_ARG_LIST] !", p_oam_agent_process_args->len);
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT8(*pp_buffer, &p_oam_agent_process_args->len, "len");
    *pp_buffer += sizeof(p_oam_agent_process_args->len);
    ...
    /* Compose OCTET_STRING VARIABLE of basic type elements */
    {
        ...
        {
            OAM_CP_PACK_CHAR8(*pp_buffer, &p_oam_agent_process_args->name[loop], "name[]");
            *pp_buffer += sizeof(Char8);
        }
    }
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_name(pp_buffer, &p_oam_agent_process_spawn_list->process_name))
    {
        return OAM_FAILURE;
    }

    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_args(pp_buffer, &p_oam_agent_process_spawn_list->arg_list))
    {
        return OAM_FAILURE;
    }
...
)
{
    ...
    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT8(*pp_buffer, &p_agent_oam_spawn_process_resp->num_failed_process, "num_failed_process");
    *pp_buffer += sizeof(p_agent_oam_spawn_process_resp->num_failed_process);
    ...
    /* Compose OCTET_STRING VARIABLE of IEs */
    {
        ...
        for (loop = 0; loop < p_agent_oam_spawn_process_resp->num_failed_process; loop++)
        {
            if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_spawn_status_list(pp_buffer, &p_agent_oam_spawn_process_resp->process_list[loop]))
            {
                return OAM_FAILURE;
            }
        }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):AGENT_OAM_SPAWN_PROCESS_RESP Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_agent_oam_spawn_process_resp(&p_agnt_msg, p_agent_oam_spawn_process_resp))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_name(pp_buffer, &p_oam_agent_process_spawn_status_list->failed_process_name))
    {
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT32(*pp_buffer, &p_oam_agent_process_spawn_status_list->err, "err");
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_oam_agent_kill_process_req->num_process] should be in range [%d <= OAM_AGENT_MAX_NUM_PROCESS] !", p_oam_agent_kill_process_req->num_process);
        return OAM_FAILURE;
    }
...
)
{
    ...
    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT8(*pp_buffer, &p_oam_agent_kill_process_req->num_process, "num_process");
    *pp_buffer += sizeof(p_oam_agent_kill_process_req->num_process);
    ...
    /* Compose OCTET_STRING VARIABLE of IEs */
    {
        ...
        for (loop = 0; loop < p_oam_agent_kill_process_req->num_process; loop++)
        {
            if (OAM_FAILURE == oam_agnt_compose_oam_agent_kill_process_list(pp_buffer, &p_oam_agent_kill_process_req->process_list[loop]))
            {
                return OAM_FAILURE;
            }
        }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):OAM_AGENT_KILL_PROCESS_REQ Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_kill_process_req(&p_agnt_msg, p_oam_agent_kill_process_req))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_name(pp_buffer, &p_oam_agent_kill_process_list->process_name))
    {
        return OAM_FAILURE;
    }
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_agent_oam_kill_process_resp->num_process] should be in range [%d <= OAM_AGENT_MAX_NUM_PROCESS] !", p_agent_oam_kill_process_resp->num_process);
        return OAM_FAILURE;
    }
...
)
{
    ...
    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT8(*pp_buffer, &p_agent_oam_kill_process_resp->num_process, "num_process");
    *pp_buffer += sizeof(p_agent_oam_kill_process_resp->num_process);
    ...
    /* Compose OCTET_STRING VARIABLE of IEs */
    {
        ...
        for (loop = 0; loop < p_agent_oam_kill_process_resp->num_process; loop++)
        {
            if (OAM_FAILURE == oam_agnt_compose_oam_agent_kill_process_status_list(pp_buffer, &p_agent_oam_kill_process_resp->fail_process_list[loop]))
            {
                return OAM_FAILURE;
            }
        }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):AGENT_OAM_KILL_PROCESS_RESP Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_agent_oam_kill_process_resp(&p_agnt_msg, p_agent_oam_kill_process_resp))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_process_name(pp_buffer, &p_oam_agent_kill_process_status_list->failed_process_name))
    {
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT32(*pp_buffer, &p_oam_agent_kill_process_status_list->err, "err");
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_file_name(pp_buffer, &p_agent_oam_get_logs_resp->filename))
    {
        return OAM_FAILURE;
    }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):AGENT_OAM_GET_LOGS_RESP Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_agent_oam_get_logs_resp(&p_agnt_msg, p_agent_oam_get_logs_resp))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    {
        OAM_LOG(OAM, OAM_WARNING, "Parameter [p_oam_agent_file_name->len] should be in range [%d <= OAM_AGENT_MAX_FILE_NAME] !", p_oam_agent_file_name->len);
        return OAM_FAILURE;
    }

    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT16(*pp_buffer, &p_oam_agent_file_name->len, "len");
    *pp_buffer += sizeof(p_oam_agent_file_name->len);
    ...
    /* Compose OCTET_STRING VARIABLE of basic type elements */
    {
        ...
        {
            OAM_CP_PACK_CHAR8(*pp_buffer, &p_oam_agent_file_name->name[loop], "name[]");
            *pp_buffer += sizeof(Char8);
        }
    }
...
)
{
    ...
    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_file_name(pp_buffer, &p_oam_agent_get_logs_cnf->filename))
    {
        return OAM_FAILURE;
    }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):OAM_AGENT_GET_LOGS_CNF Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_get_logs_cnf(&p_agnt_msg, p_oam_agent_get_logs_cnf))
    {
        oam_mem_free(p_msg, &err);
...
)
{
    ...
    /* Compose parameter of basic type */
    OAM_CP_PACK_UINT64(*pp_buffer, &p_agent_oam_im_ok_ind->cksum, "cksum");
    *pp_buffer += sizeof(p_agent_oam_im_ok_ind->cksum);

    /* Compose IE */
    if (OAM_FAILURE == oam_agnt_compose_oam_agent_dir_path(pp_buffer, &p_agent_oam_im_ok_ind->bin_path))
    {
        return OAM_FAILURE;
    }
...
)
{
    ...
    OAM_LOG(OAM, OAM_INFO, "Src(%u)->Dst(%u):AGENT_OAM_IM_OK_IND Instance %u", src_module_id, dest_module_id, instance_id);
    ...
    p_agnt_msg = p_agnt_msg + LOWER_LAYER_INTERFACE_HEADER_SIZE;
    if (OAM_FAILURE == oam_agnt_compose_agent_oam_im_ok_ind(&p_agnt_msg, p_agent_oam_im_ok_ind))
    {
        oam_mem_free(p_msg, &err);
