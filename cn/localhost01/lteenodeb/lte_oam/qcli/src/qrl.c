...
static	buffer_t	*__newbuffer( const edit_t *M )
{
	buffer_t	*B = (buffer_t *)malloc( sizeof(buffer_t) );
        /* coverity 36034 36033 36032 36031 klockwork 12544 */
        if( !B ){
         return B;
        }
	B->size = B->count = 0;
	B->buffer = (char *)malloc( B->size + 1);
        /* klockwork 12545 coverity 35452 */
        if(!(B->buffer)){
         free(B);
         B = 0;
         return B;
...
{
    /*  coverity 56826 fix start */
	if( -1 == write( B->fd, B->buffer, B->count ))
    {
        return;
    }
    /*  coverity 56826 fix end */
	free( B->buffer );
	free( B );
}
...
static void	__addhistory( edit_t *M, const char *s )
{
	...
	T = (item_t *) malloc( sizeof(item_t) );
        /* klockwork 12546 coverity 35449*/
        if(!T)
        {
         return;
        }
		...
		void *arg )
{
	...
	M->buffer = (char *) malloc (M->size + 1);
        /* klockwork 12547 coverity 35453 */
        if( !(M->buffer))
        {
         return;
        }
...
{
	int	i, e = 0;
/* never got why we alway got the undefined cur_term in aarch64, this is a bypass. */
#ifdef AARCH64
	int cur_term = 0;
#endif
	const sequence_t
	...
	M = (edit_t *) malloc( sizeof(edit_t) );
        /* klockwork 12548 coverity 35455 */
        if(M == 0)
        {
        return 0;
        }
