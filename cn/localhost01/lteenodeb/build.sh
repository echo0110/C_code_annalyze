...
# put the last git commit id into a header file
commitid_pref="#define commit_id \""
commitid=`git rev-parse --short HEAD`
commitid=$commitid_pref$commitid\"
echo $commitid >  common/include/lteversion.h
...
if [ "enable" == $ENDC_FEAT ];   then 
	#sed will force whole l3 recompiled， besides ENDC_ENABLE is already turned on in Makefiles
...
echo DUPLEXING_MODE=$DUPLEXING_MODE PLATFORM=$PLATFORM PLATFORM_PATH=$PLATFORM_PATH SCTP_STACK=$SCTP_STACK

#sed will force whole l3 recompiled， besides LOCKLESS_IMPL is already turned off in Makefiles
...
if [ "1" == $ASTRI_FLEXRAN ]
	...
	sed -i "s/UFLEXRAN_FAPI_LOGGING/DFLEXRAN_FAPI_LOGGING/g" $LTE_ROOT/make.inc
    else
        sed -i "s/UFAPI_4_0_COMPLIANCE/DFAPI_4_0_COMPLIANCE/g" $LTE_ROOT/make.inc
        sed -i "s/DFLEXRAN/UFLEXRAN/g" $LTE_ROOT/make.inc
	sed -i "s/DFLEXRAN_FAPI_LOGGING/UFLEXRAN_FAPI_LOGGING/g" $LTE_ROOT/make.inc
fi
...
then
    ...
    if [ "true" == $LINUX_PC_TEST ]
    then	
	#start build TEST code#	
	echo "*************************** BUILDING TEST         *********************"	
	if [ $1 == $L2WITHADAPTER ]; then
		make  L2_BUILD_FLAG=1 -C $LTE_ROOT/../../TEST
	elif [ $1 == $ENBLTERRM ]; then
		make  RRM_BUILD_FLAG=1 -C $LTE_ROOT/../../TEST
	elif [ $1 == $PDCP ]; then
		make  PDCP_BUILD_FLAG=1 -C $LTE_ROOT/../../TEST
	elif [ $1 == $GTPU ]; then
		make  GTPU_BUILD_FLAG=1 -C $LTE_ROOT/../../TEST
	elif [ $1 == $ENBSON ]; then
		make  SON_BUILD_FLAG=1 -C $LTE_ROOT/../../TEST
	else
		make  -C $LTE_ROOT/../../TEST
        fi        
...
if [ $1 == $ENBSON ]; then
    ...
    if [ $LTE_CC == "arm-linux-gnueabi-gcc" ]
        then
        cp -f $LTE_ROOT/lte_rrc/platform_lib/ASN_LIB/ARM/*.so $LTE_ROOT/lte_son/build/lib/
        elif [ $LTE_CC == aarch64-linux-gnu-gcc ]
            then
            cp -f $LTE_ROOT/lte_rrc/platform_lib/ASN_LIB/arm64/*.so $LTE_ROOT/lte_son/build/lib/
        else
...
NBIOT_FEAT=enable
    ...
    then
	#sed will force whole l3 recompiled， besides HIGH_SCALABILITY_L3_MAX_UE is already turned on in Makefiles
    ...
    cp -f $LTE_ROOT/../lib/$PLATFORM_PATH/libasn1per.so /usr/lib/
    cp -f $LTE_ROOT/../lib/$PLATFORM_PATH/libasn1rt.so /usr/lib/
...
# M2AP Changes - End
    ...
    check_failure
    cp -f config_file debug/bin/.
    check_failure
    cd -
    echo ""
    echo "Copying L3 binary to ../bin folder"
	cp -f $LTE_ROOT/lte_rrc/runner/debug/bin/rrc_entity $LTE_ROOT/../bin/
    echo ""
fi
if [ $1 == $FRAMEWORK ]; then
...
if [ $1 == $CLEANUP ]; then
    ...
    exit 1
		...
		#start build TEST code#
		echo "*************************** CLEANING TEST         *********************"
		make clean -C $LTE_ROOT/../../TEST
		check_failure
		echo "*************************** END CLEANING TEST         *********************"
	     fi
