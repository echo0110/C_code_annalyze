                         ...
                         unsigned int   *p_scode_id )
{
    ...
    if( p_dest_ip )
    {
        p_dest_ip[0] = ntohl( saddr_in6.sin6_addr.__u6_addr.__u6_addr32[0] );
        p_dest_ip[1] = ntohl( saddr_in6.sin6_addr.__u6_addr.__u6_addr32[1] );
        p_dest_ip[2] = ntohl( saddr_in6.sin6_addr.__u6_addr.__u6_addr32[2] );
        p_dest_ip[3] = ntohl( saddr_in6.sin6_addr.__u6_addr.__u6_addr32[3] );
  ...
  unsigned char       b_is_iovec )
{
    ...
    entity_addr.sin6_addr.__u6_addr.__u6_addr32[0] = htonl( p_dest_ip[0] );
    entity_addr.sin6_addr.__u6_addr.__u6_addr32[1] = htonl( p_dest_ip[1] );
    entity_addr.sin6_addr.__u6_addr.__u6_addr32[2] = htonl( p_dest_ip[2] );
    entity_addr.sin6_addr.__u6_addr.__u6_addr32[3] = htonl( p_dest_ip[3] );
    ...
    if( SIGTRAN_FALSE  == b_is_iovec )
    {
         ...
         * If the data passed is not in the form of iovec.
         */
        if( ( nsent = sendto( sockfd, ( char * )p_buf, buffer_len, MSG_DONTROUTE,
                              ( struct sockaddr * )&entity_addr,
  ...
  sigtran_U32bit         scope_id )
{
    ...
    if( SIGTRAN_NULL != p_serv_ip )
    {
        ...
        saddr.sin6_addr.__u6_addr.__u6_addr32[0]   = htonl( p_serv_ip[0] );
        saddr.sin6_addr.__u6_addr.__u6_addr32[1]   = htonl( p_serv_ip[1] );
        saddr.sin6_addr.__u6_addr.__u6_addr32[2]   = htonl( p_serv_ip[2] );
        saddr.sin6_addr.__u6_addr.__u6_addr32[3]   = htonl( p_serv_ip[3] );
    ...
    sigtran_U32bit      num_addrs )
{
    ...
    * /
    ...
    if( ( htonl( s_addr1 = hostGetByName( p_hostname ) ) ) == ( sigtran_U32bit )0xffffffff )
    {
        return ( 0 );
    }
