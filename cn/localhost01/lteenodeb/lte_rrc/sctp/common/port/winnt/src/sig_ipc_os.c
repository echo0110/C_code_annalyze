  ...
  unsigned char   b_is_iovec )
{
    ...
    {
        p_buffers = ( LPWSABUF )p_buf;
        /*
         * If the data passed is in the form of iovec.
         */
        ret_val = WSASend( sockfd, p_buffers, nbytes, &bytes_sent, 0, 0, 0 );
  ...
  unsigned char       b_is_iovec )
{
    ...
    entity_addr.sin_port        = htons( ( unsigned short int )dest_port );
    ...
    else
    {
         ...
         * If the data passed is in the form of iovec.
         */
        ret_val = WSASendTo( sockfd, p_buffers, buffer_len, &bytes_sent, 0,
                             ( const struct sockaddr * )&entity_addr, sizeof( entity_addr ), NULL, NULL );
  ...
  sigtran_U16bit         serv_port )
{
    ...
    if( ( sock = socket( AF_INET, SOCK_RAW, IPPROTO_SCTP ) ) < 0 )
    {
        return 0;
    }
  ...
  unsigned char       b_is_iovec )
{
    ...
    else
    {
         ...
         * If the data passed is in the form of iovec.
         */
        ret_val = WSASendTo( sockfd, p_buffers, nbytes, &bytes_sent, 0,
                             ( const struct sockaddr * )&entity_addr, sizeof( entity_addr ), NULL, NULL );
  ...
  sigtran_U32bit         scope_id )
{
    ...
    if( ( sock = socket( AF_INET6, SOCK_RAW, IPPROTO_SCTP ) ) < 0 )
    {
        return 0;
    }
