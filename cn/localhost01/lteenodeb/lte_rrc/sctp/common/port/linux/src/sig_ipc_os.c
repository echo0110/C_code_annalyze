  ...
  sigtran_U16bit         serv_port )
{
    ...
    /* CSR 69326 fix starts */
    else if( ( sigtran_S16bit )sockfd == 0 )
    {
        close( sockfd );
...
( sigtran_U32bit         serv_ip,
  sigtran_U16bit          serv_port )
{
    /*gcc warning removal of unused variable - neha */
  ...
  unsigned char   b_is_iovec )
{
    ...
    struct msghdr   msg;
    /* SPR 21107 changes */
    sigtran_memset( &msg, 0, sizeof( struct msghdr ) );
    /* SPR 21107 changes */
    /* SPR 21003 Starts */
    int             bytes_written = 0;
    /* SPR 21003 Ends */
                     ...
                     unsigned int   *len_read )
{
    ...
    /* SPR 21003 Starts */
    else if( read_flag == SIG_RECV_DONTWAIT )
    {
        flag = MSG_DONTWAIT;
    }
    ...
    if( ( msgsize = recvfrom( sockfd, ( char * )pbuf, len_to_read, flag,
                              ( struct sockaddr * )&saddr, ( socklen_t * )&saddrlen ) ) <= 0 )
    {
        *len_read = 0;
        ...
        if( errno == EAGAIN )
        {
            return 1; /* return 1 so that this case can be dealt as success case.*/
    ...
    if( sizeof( struct sockaddr_in ) == saddrlen )
    {
        if( ip_addr )
        {
            *ip_addr = ntohl( saddr.sin_addr.s_addr );
                         ...
                         unsigned int   *p_scode_id )
{
    ...
    if( ( msgsize = recvfrom( sockfd, ( char * )pbuf, len_to_read, flag,
                              ( struct sockaddr * )&saddr_in6, ( socklen_t * )&saddrlen ) ) < 0 )
    {
        *len_read = 0;
        return 0;
    }
    ...
    if( p_dest_ip )
    {
        p_dest_ip[0] = ntohl( saddr_in6.sin6_addr.s6_addr32[0] );
        p_dest_ip[1] = ntohl( saddr_in6.sin6_addr.s6_addr32[1] );
        p_dest_ip[2] = ntohl( saddr_in6.sin6_addr.s6_addr32[2] );
        p_dest_ip[3] = ntohl( saddr_in6.sin6_addr.s6_addr32[3] );
  ...
  unsigned char       b_is_iovec )
{
    ...
    entity_addr.sin6_addr.s6_addr32[0] = htonl( p_dest_ip[0] );
    entity_addr.sin6_addr.s6_addr32[1] = htonl( p_dest_ip[1] );
    entity_addr.sin6_addr.s6_addr32[2] = htonl( p_dest_ip[2] );
    entity_addr.sin6_addr.s6_addr32[3] = htonl( p_dest_ip[3] );
  ...
  sigtran_U32bit         scope_id )
{
    ...
    if( SIGTRAN_NULL != p_serv_ip )
    {
        ...
        saddr.sin6_addr.s6_addr32[0]   = htonl( p_serv_ip[0] );
        saddr.sin6_addr.s6_addr32[1]   = htonl( p_serv_ip[1] );
        saddr.sin6_addr.s6_addr32[2]   = htonl( p_serv_ip[2] );
        saddr.sin6_addr.s6_addr32[3]   = htonl( p_serv_ip[3] );
...
**
**  INPUTS
**      serv_port:  port of the ICMP ipv6 server
**
**  RETURNS
**      server fd if succesful
**      0 otherwise
...
**
**  INPUTS
**      serv_port:  port of the udp server
**
**  RETURNS
**      server fd if succesful
**      0 otherwise
...
**
**    Description       : It determines the status of one or more sockets.
**
**    Returns           : Number of sockets in fd_set structure.
**
**
******************************************************************************/
...
**
**    Description       : This function resolve hostname
**
**    Returns           : sigtran_U32bit
**
******************************************************************************/
    ...
    sigtran_U32bit      num_addrs )
{
    ...
    hostptr = gethostbyname( ( char * )p_hostname );
...
**
**    Description       : Retrieves the standard hostname for local computer.
**
**    Returns           : If no error returns zero
**
**
******************************************************************************/
...
{
    return gethostname( ( char * )p_hostname, host_len );
 ...
 * **
 * **  DESCRIPTION:  Returns/Generates  random number
 * **
 ...
 * **
 * **  DESCRIPTION:  Initialises seed value for random number generator
 * **
 * **  RETURN VALUE: None
 * ******************************************************************************/
 ...
 *
 *    DESCRIPTION: Fills the csmg header.
 *
 *    RETURN VALUE:
 *       void
 **************************************************************************/
