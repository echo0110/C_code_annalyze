...
#elif defined(GL_SOLARIS) || defined(GW_LINUX)
    if( 6 < backlog )
    {
        return GL_FAILURE;
    }
...
#elif GL_VXWORKS
    if( ERROR == ( num_bytes = sendto( sockid, buff, nbytes, flags, p_remote_destaddr,
                                       destaddr_len ) ) )
#else
    if( GL_ERROR == ( num_bytes = sendto( sockid, ( gl_pvoid_t )buff, ( gw_U32bit )nbytes,
                                          flags, p_remote_destaddr, ( gw_U32bit )destaddr_len ) ) )
#endif
    {
        return GL_ERROR;
    }
...
#elif GL_VXWORKS
    if( ERROR == ( num_bytes = recvfrom( sockid, buff, nbytes, flags, p_local_srcaddr,
                                         ( gw_U32bit )p_addrlen ) ) )
#elif GL_LINUX
    if( GL_ERROR == ( num_bytes = recvfrom( sockid, buff, nbytes, flags,
                                            p_local_srcaddr, ( gw_U32bit * )p_addrlen ) ) )
#else
    if( GL_ERROR == ( num_bytes = recvfrom( sockid, ( gl_pvoid_t )buff, ( gw_U32bit )
                                            nbytes, flags, p_local_srcaddr, ( gw_U32bit * )
                                            p_addrlen ) ) )
#endif
    {
        return GL_ERROR;
...
#elif defined(GL_SOLARIS) || defined(GW_LINUX)
    if( GL_ERROR == ( num_bytes = send( sockid, ( gl_pvoid_t )buff, ( gw_U32bit )nbytes,
                                        flags ) ) )
#elif GL_OSE
...
#else
    if( GL_ERROR == ( num_bytes = recv( sockid, ( gl_pvoid_t )buff, ( gw_U32bit )nbytes,
                                        flags ) ) )
#endif
    {
        return GL_ERROR;
...
#else
    if( GL_ERROR == setsockopt( sockid, lev, opt, ( gl_pvoid_t )p_optval,
                                ( gw_U32bit )optlen ) )
#endif
    {
        return GL_FAILURE;
...
{
    gw_U32bit addr;
#if defined(GL_SOLARIS) || defined(GW_LINUX)
...
#else
    if( ( gethostname( name, ( gw_S32bit )name_len ) ) != 0 )
#endif
    {
        return GL_ERROR;
    }
...
/****************************************************************************/
{
    ...
    {
#if !(defined GL_OSE) && !(defined GL_LINUX)
