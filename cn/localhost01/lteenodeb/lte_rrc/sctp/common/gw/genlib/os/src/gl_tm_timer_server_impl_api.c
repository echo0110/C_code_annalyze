...
/****************************************************************************/
{
     ...
     */
    if( GL_FAILURE == gl_create_semaphore( &gl_tm_timer_task, ( gw_U32bit )0,
                                           ( gw_U32bit )1, p_ecode ) )
    {
     ...
     */
    if( GL_FAILURE == gl_create_semaphore( &gl_tm_main_event, ( gw_U32bit )0,
                                           ( gw_U32bit )1, p_ecode ) )
    {
        GL_INC_TM_G_MUTX_FAIL()
    ...
    if( GL_FAILURE == gl_create_semaphore( &gl_tm_input_event, ( gw_U32bit )0,
                                           ( gw_U32bit )1, p_ecode ) )
    {
        GL_INC_TM_G_MUTX_FAIL()
    ...
    if( GL_FAILURE == gl_thm_create( ( gl_fn_ptr_t )gl_tm_timer_server,
                                     p_thread_info, ( gl_pvoid_t )GL_NULL, p_ecode ) )
    {
        return GL_FAILURE;
    }
