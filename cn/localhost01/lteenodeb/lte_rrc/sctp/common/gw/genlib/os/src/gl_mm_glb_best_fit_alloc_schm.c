...
*                                            time errors.
* 04/10/2001    Ravi Jindal     SPR-2751     Remove warnings with gcc44 -m64 -g,
*                                            cc,CC on Solaris.
...
*                                            gl_pvoid_t.
*
* 04/01/2002    Prashant        SPR-2751     Remove warnings with gcc44 -m64 -g,
...
* 29 Jun, 2011  Vaibhav Banga   SPR -20940       kw fix for CSR 4756
*
...
/****************************************************************************/
{
     ...
     */
    if( ( gl_pvoid_t )GL_NULL == ( g_a_p_glb_pool_table[i] =
...
#endif
    p_pool->p_hdr_region = ( gl_mm_buffer_hdr_t * )( ( gw_U8bit * )p_pool +
                                                     sizeof( gl_mm_pool_t ) );
     ...
     */
    p_buf_start = ( gl_mm_check_mem_overwrite_t * )( ( gw_U8bit * )p_buf_start +
        ...
        if( p_temp->buf_size >= buf_size )
        {
            ...
            if( p_temp->p_first != ( gl_pvoid_t )GL_NULL )
            {
                        ...
                        p_temp->specs.num_regions > 0 )
                {
                    ...
                    if( p_alloc->pool_id < GL_MM_MAX_COMMON_POOL_QTY )
...
/****************************************************************************/
{
...
#ifdef GL_MM_ADD_MEM_OVERW_OPR
    p_buf_start = ( gl_mm_check_mem_overwrite_t * )
                  ( ( gw_U8bit * )buf_ptr - sizeof( gl_mm_check_mem_overwrite_t ) );
    ...
    p_buf_end = ( gl_mm_check_mem_overwrite_t * )
                ( ( gw_U8bit * )buf_ptr + g_a_p_glb_pool_table[pool_id]->buf_size );
    ...
    pool_id = ( ( gl_mm_check_mem_overwrite_t * )( ( gw_U8bit * )buf_ptr -
    ...
    /* Klockworks 8.0 fix start */
    if( p_hdr->pool_id >= GL_MM_MAX_COMMON_POOL_QTY )
    {
        return GL_FAILURE;
    }
...
/****************************************************************************/
{
            ...
            p_prev_pool = p_temp, p_temp = p_next_pool )
    {
        ...
        if( p_temp->buf_size == buf_size )
        {
            ...
            for( i = 0; i < GL_MM_MAX_COMMON_POOL_QTY; i++ )
            {
                if( p_temp == g_a_p_glb_pool_table[i] )
                {
