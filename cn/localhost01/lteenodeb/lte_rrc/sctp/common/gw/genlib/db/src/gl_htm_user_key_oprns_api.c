...
/****************************************************************************/
{
            ...
            || GL_NULL == g_a_p_hashtable[hash_table_id] )
    {
        ...
        *p_p_element = GL_NULL; /* KW Fix : M3UA 6.1 */
        *p_ecode = GL_HTM_ERR_INV_HASH_TABLE;
        return GL_FAILURE;
    }
...
/*****************************************************************************/
{
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for( bucket_index = p_handle->bucket_number - 1;
                        bucket_index < g_a_p_hashtable[hash_table_id]->bucket_qty;
                        bucket_index-- )
                {
                    p_bucket = g_a_p_hashtable[hash_table_id]->a_bucket_array
                               + bucket_index;
    ...
    {
        if( ( p_temp_node ) && ( ( GL_NULL != p_temp_node->p_next ) || ( GL_TRUE == is_first_node ) ) )
        {
            /*
