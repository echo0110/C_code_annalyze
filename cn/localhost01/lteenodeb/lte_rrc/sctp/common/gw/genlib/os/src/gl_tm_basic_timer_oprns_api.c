...
/****************************************************************************/
{
    ...
    *p_timer_id = get_free_timer( timer_list_id );
    ...
    else
    {
         ...
         */
        if( GL_FAILURE == gl_release_semaphore( &gl_tm_input_event, p_ecode ) )
        {
            return GL_FAILURE;
        }

        if( GL_FAILURE == gl_release_semaphore( &gl_tm_timer_task, p_ecode ) )
        {
            return GL_FAILURE;
        }
    ...
    *p_p_timer_buf = a_timer_app_list[timer_list_id].
    ...
    else
    {
        ...
        gl_tm_send_msg_to_server( GL_TM_STOP_TIMER, ( gl_timer_list_t )
                                  ( timer_list_id + 1 ), timer_id, ( gw_U8bit )GL_NULL, ( gw_U32bit )
                                  GL_NULL );
...
/****************************************************************************/
{
    ...
    else
    {
         ...
         */
        if( GL_FAILURE == gl_release_semaphore( &gl_tm_main_event, &ecode ) )
        {
            return;
        }

        if( GL_FAILURE == gl_release_semaphore( &gl_tm_timer_task, &ecode ) )
        {
            return;
        }
    }
    ...
    else
    {
        for( i = 0, count = 0; i < a_timer_app_list[timer_list_id].max_timer_qty; i++ )
        {
                    ...
                    != GL_NULL )
            {
                ...
                a_p_timer_buf[count++] = a_timer_app_list[timer_list_id].
                                         a_timer_index[i].p_timer_buf;
                set_free_timer( timer_list_id, ( gl_timer_id_t )( i + 1 ) );
            }
        }
        ...
        *p_timer_qty = count;
        ...
        gl_tm_send_msg_to_server( GL_TM_STOP_ALL_TIMER, ( gl_timer_list_t )
                                  ( timer_list_id + 1 ), ( gl_timer_id_t )GL_NULL, ( gw_U8bit )GL_NULL,
                                  ( gw_U32bit )GL_NULL );
...
/****************************************************************************/
{
    ...
    for( i = 0; i < GL_TM_MAX_TIMER_LIST; i++ )
    {
        ...
        {
            *( p_timer_list_ids + num_timer_list_ids ) = i;
            num_timer_list_ids++;
        }
    }
