...
#SCTP_DEFINES += -DSCTP_INET6_SUPPORT
...
#For DEBUG SUPPORT
SCTP_DEFINES += -DSCTP_DEBUG_ENABLED

#For ERROR SUPPORT
SCTP_DEFINES += -DSCTP_ERROR_ENABLED

#For STATS SUPPORT
#SCTP_DEFINES += -DSCTP_STATS_ENABLED
...
#SCTP_DEFINES += -DSCTP_ASSOC_STATS_RECOVERY_SUPPORT

#For separate name resolver thread
#SCTP_DEFINES += -DSCTP_NAME_RESOLV_THREAD

#For a separate time thread, these flags must be enabled.
#SCTP_DEFINES += -DSIGTRAN_LOCAL_TIME_THREAD
...
#SCTP_DEFINES += -DSIGTRAN_CLOCK_MONOTONIC    
#SIG_COMMON_FLAGS += -DSIGTRAN_CLOCK_MONOTONIC    

#For socket interface
SCTP_DEFINES += -DSCTP_UI_SOCK_API

#For message based interface with su
#SCTP_DEFINES += -DSCTP_UI_SU_MESSAGEBASED

#For message based interface with sm
#SCTP_DEFINES += -DSCTP_UI_SM_MESSAGEBASED

#Send data with the data pending indication in msg i/f with SU
#SCTP_DEFINES += -DSCTP_SEND_DATA_WITH_IND

#For C Structure based message interface
#SCTP_DEFINES += -DSCTP_STRUCT_BASED

#For redundancy support, message based should also be enabled
#SCTP_DEFINES += -DSCTP_RED_SUPPORT 

#SCTP_DEFINES += -DSCTP_CLI_RECV_COM_THREAD 

#For enabling dynamic ip reconfiguration (ASCONF)
#SCTP_DEFINES += -DSCTP_DYNAMIC_IP_SUPPORT

#For Configuration of TTL and DSCP
#SCTP_DEFINES += -DSCTP_IP_HEADER_PARAMS

#For common polling & receiving thread 
#SCTP_DEFINES += -DSCTP_CMN_RECV_POLL_THREAD

#For preserving the byte order of ppid as provided by the host 
#SCTP_DEFINES += -DSCTP_HOST_PPID

SCTP_DEFINES += $(SCTP_BASIC_FLAGS) $(SIG_COMMON_FLAGS) $(SCTP_CSPL_CFLAGS)

#########################################################
#INCLUDE SERACH PATHS
#########################################################
SCTP_INCLUDES=  -I$(COM_BGTD_PATH)/stack/co/h\
